...
TEST_F(ScalarEvolutionExpanderTest, ExpandPtrTypeSCEV) {
  ...
  AllocaInst *Alloca =
      new AllocaInst(I32Ty, DL.getAllocaAddrSpace(), "alloca", Br->getIterator());
  ...
  CastInst *CastA =
      CastInst::CreateBitOrPointerCast(Gep0, I8PtrTy, "bitcast1", Br->getIterator());
  ...
  CmpInst *Cmp = CmpInst::Create(Instruction::ICmp, CmpInst::ICMP_ULT,
                                 UndefValue::get(I8PtrTy), CastA, "cmp", Br->getIterator());
  SelectInst *Sel = SelectInst::Create(Cmp, Gep1, Gep2, "select", Br->getIterator());
  CastInst *CastB =
      CastInst::CreateBitOrPointerCast(Sel, I32PtrTy, "bitcast2", Br->getIterator());
