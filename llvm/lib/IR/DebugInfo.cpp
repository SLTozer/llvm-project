...
bool at::calculateFragmentIntersect(
  ...
  auto CalculateFragmentIntersectImpl = [&](const auto *AssignRecord) -> bool {
    ...
    DIExpression::FragmentInfo SliceOfVariable(SliceSizeInBits,
                                               NewOffsetInBits);
  ...
  return CalculateFragmentIntersectImpl(
      cast<const DbgAssignIntrinsic *>(Assign));
}
...
bool AssignmentTrackingPass::runOnFunction(Function &F) {
  ...
  for (auto &P : DbgDeclares) {
    ...
    SmallVector<DPValue *> DPMarkers = at::getDPAssignmentMarkers(Alloca);
    (void)Markers;
