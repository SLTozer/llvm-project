...
void DroppedVariableStats::setup() {
  ...
  InlinedAts.push_back({DenseMap<StringRef, DenseMap<VarID, DebugLoc>>()});
}
...
void DroppedVariableStats::calculateDroppedStatsAndPrint(
  ...
  if (It == InlinedAts.back().end())
    return;
  DenseMap<VarID, DebugLoc> &InlinedAtsMap = It->second;
...
bool DroppedVariableStats::updateDroppedCount(
    DebugLoc DbgLoc, const DIScope *Scope, const DIScope *DbgValScope,
    DenseMap<VarID, DebugLoc> &InlinedAtsMap, VarID Var,
...
void DroppedVariableStats::run(DebugVariables &DbgVariables, StringRef FuncName,
  ...
  if (Before)
    InlinedAtsMap.try_emplace(FuncName, DenseMap<VarID, DebugLoc>());
...
bool DroppedVariableStats::isInlinedAtChildOfOrEqualTo(
    DebugLoc InlinedAt, DebugLoc DbgValInlinedAt) {
