...
public:
  ...
  loadInlocs(MachineBasicBlock &MBB, ValueTable &MLocs, DbgOpIDMap &DbgOpStore,
    ...
    for (const auto &Var : VLocs) {
      ...
      if (LastUseBeforeDef) {
        addUseBeforeDef(Var.first, Var.second.Properties, DbgOps,
                        LastUseBeforeDef);
        continue;
      }
      ...
      auto NewValue = ResolvedDbgValue{ResolvedDbgOps, Var.second.Properties};
...
bool InstrRefBasedLDV::vlocJoin(
  ...
  for (auto &V : Values) {
    ...
    // If both values have equal non-empty IDs, they refer to the same value
