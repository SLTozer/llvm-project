...
public:
  ...
  struct LocAndProperties {
    SmallVector<ResolvedDbgOp> Locs;
    DbgValueProperties Properties;

    LocAndProperties(SmallVectorImpl<ResolvedDbgOp>& Locs, DbgValueProperties Properties) :
        Locs(Locs.begin(), Locs.end()), Properties(Properties) {}

    auto loc_indices() const {
      return map_range(make_filter_range(Locs,
        [](const ResolvedDbgOp& Op) { return !Op.IsConst; }),
  ...
  struct UseBeforeDef {
    /// Value of this variable, fully def'd in block.
    SmallVector<DbgOp> Values;
    /// Identity of this variable.
    DebugVariable Var;
    /// Additional variable properties.
    DbgValueProperties Properties;
    UseBeforeDef(SmallVectorImpl<DbgOp>& Values, DebugVariable Var,
                 DbgValueProperties Properties) :
        Values(Values.begin(), Values.end()), Var(Var), Properties(Properties)
    {}
  };
  ...
  loadInlocs(MachineBasicBlock &MBB, ValueTable &MLocs, DbgOpIDMap &DbgOpStore,
    ...
    for (auto &VLoc : VLocs)
      if (VLoc.second.Kind == DbgValue::Def)
        for (DbgOpID OpID : VLoc.second.getDbgOpIDs())
            if (!OpID.ID.IsConst)
                ValueToLoc.insert({DbgOpStore.find(OpID).ID, LocIdx::MakeIllegalLoc()});
    ...
    for (const auto &Var : VLocs) {
      ...
      // and we can't recover as an entry value, we set IsValidValue=false and
      // skip this variable.
      for (DbgOpID ID : Var.second.getDbgOpIDs()) {
      ...
      if (LastUseBeforeDef) {
        addUseBeforeDef(Var.first, Var.second.Properties, DbgOps, LastUseBeforeDef);
        continue;
      }
      ...
      for (const ResolvedDbgOp &Op : ResolvedDbgOps)
        if (!Op.IsConst)
          ActiveMLocs[Op.Loc].insert(Var.first);
      auto NewValue = LocAndProperties{ResolvedDbgOps, Var.second.Properties};
      auto Result = ActiveVLocs.insert(std::make_pair(Var.first, NewValue));
      if (!Result.second)
        Result.first->second = NewValue;
      PendingDbgValues.push_back(
          MTracker->emitLoc(ResolvedDbgOps, Var.first, Var.second.Properties));
    }
  ...
  void checkInstForNewValues(unsigned Inst, MachineBasicBlock::iterator pos) {
    ...
    for (auto &Use : MIt->second) {
      ...
      for (DbgOp &Op : Use.Values) {
        ...
        LocIdx NewLoc = ValueToLoc[Op.ID];
        if (NewLoc.isIllegal())
          break;
      ...
      PendingDbgValues.push_back(MTracker->emitLoc(DbgOps, Use.Var, Use.Properties));
    }
  ...
  bool recoverAsEntryValue(const DebugVariable &Var,
    ...
    PendingDbgValues.push_back(emitMOLoc(MO, Var, {NewExpr, Prop.Indirect, false}));
    return true;
  }
  ...
  void redefVar(const MachineInstr &MI) {
    ...
    if (MI.isUndefDebugValue() || all_of(MI.debug_operands(), [](const MachineOperand &MO) {
      return !MO.isReg();
    })) {
      auto It = ActiveVLocs.find(Var);
      if (It != ActiveVLocs.end()) {
        for (ResolvedDbgOp &Op : It->second.Locs)
          if (!Op.IsConst)
            ActiveMLocs[Op.Loc].erase(Var);
  ...
  void redefVar(const MachineInstr &MI, const DbgValueProperties &Properties,
    ...
    SmallVector<ResolvedDbgOp> OldOps;

    // Erase any previous location,
    auto It = ActiveVLocs.find(Var);
    if (It != ActiveVLocs.end()) {
      OldOps = It->second.Locs;
      for (ResolvedDbgOp& Op : It->second.Locs)
        if (!Op.IsConst) {
          ActiveMLocs[Op.Loc].erase(Var);
        }
    ...
    for (ResolvedDbgOp &Op : NewLocs) {
      ...
      if (MTracker->readMLoc(NewLoc) != VarLocs[NewLoc.asU64()]) {
        ...
        for (auto LostMLoc : LostMLocs) {
          for (auto Var : LostMLoc.second)
            ActiveMLocs[LostMLoc.first].erase(Var);
        }
    ...
    if (It == ActiveVLocs.end()) {
      ActiveVLocs.insert(
          std::make_pair(Var, LocAndProperties(NewLocs, Properties)));
    } else {
      It->second.Locs.assign(NewLocs);
  ...
  void clobberMloc(LocIdx MLoc, ValueIDNum OldValue,
    ...
    for (auto &Var : ActiveMLocIt->second) {
      ...
      if (!NewLoc) {
        for (const ResolvedDbgOp &Op : ActiveVLocIt->second.Locs) {
          if (!Op.IsConst && Op.Loc != MLoc)
            LostMLocs[Op.Loc].insert(Var);
        }
        ActiveVLocs.erase(ActiveVLocIt);
      } else {
        ActiveVLocIt->second.Locs = DbgOps;
        NewMLocs.insert(Var);
      }
    }
    ...
    for (auto &LocVarIt : LostMLocs) {
      ...
      for (auto &Var : LocVarIt.second) {
        assert(LostMLocIt->second.contains(Var) &&
              "Variable was using this MLoc, but does not appear in "
              "ActiveMLocs?");
        LostMLocIt->second.erase(Var);
      }
    }
  ...
  void transferMlocs(LocIdx Src, LocIdx Dst, MachineBasicBlock::iterator Pos) {
    ...
    for (auto &Var : MovingVars) {
      ...
      MachineInstr *MI = MTracker->emitLoc(ActiveVLocIt->second.Locs,
        Var, ActiveVLocIt->second.Properties);
...
void DbgOpID::dump(const MLocTracker *MTrack, const DbgOpIDMap *OpStore) const {
  if (!OpStore) {
    ...
    OpStore->find(*this).dump();
  }
}
void DbgValue::dump(const MLocTracker *MTrack, const DbgOpIDMap *OpStore) const {
  if (Kind == NoVal) {
    dbgs() << "NoVal(" << BlockNo << ")";
...
MachineInstrBuilder MLocTracker::emitLoc(const SmallVectorImpl<ResolvedDbgOp> &DbgOps,
  ...
  // TODO: Dev assertion - we're trying to make sure that either there is a
  // fully defined list of ops, or there is an empty list.
#ifdef EXPENSIVE_CHECKS
  assert(all_of(DbgOps, [](const ResolvedDbgOp &Op) {
  ...
  for (size_t Idx = 0; Idx < Properties.getLocationOpCount(); ++Idx) {
    ...
    if (LocID >= NumRegs) {
      ...
      if (Offset == 0) {
        ...
        if (!Properties.IsVariadic && !Properties.Indirect)
          Indirect = true;
        else
...
bool InstrRefBasedLDV::transferDebugValue(const MachineInstr &MI) {
  ...
  if (MI.isDebugValueList()) {
    SmallVector<DbgOpID> EmptyOpIDs;
    SmallVector<ResolvedDbgOp> EmptyResolvedOps;
    if (VTracker)
      VTracker->defVar(MI, Properties, EmptyOpIDs);
    if (TTracker)
      TTracker->redefVar(MI, Properties, EmptyResolvedOps);
    return true;
  }
  ...
  if (VTracker) {
    ...
    if (MI.isUndefDebugValue()) {
      ...
      for (const MachineOperand &MO : MI.debug_operands()) {
        ...
        // debug values.
        if (MO.isReg()) {
          DebugOps.push_back(DbgOpStore.insert(
            DbgOp(MTracker->readReg(MO.getReg()))));
        }
...
bool InstrRefBasedLDV::transferDebugInstrRef(MachineInstr &MI,
  ...
  if (!FoundLoc && NewID && NewID->getBlock() == CurBB &&
    ...
    TTracker->addUseBeforeDef(V, {MI.getDebugExpression(), false, false}, UseBeforeDefLocs,
                              NewID->getInst());
  }
...
bool InstrRefBasedLDV::transferSpillOrRestoreInst(MachineInstr &MI) {
  ...
  if (isLocationSpill(MI, MF, Reg)) {
    ...
    auto DoTransfer = [&](Register SrcReg, unsigned SpillID) {
      ...
      LocIdx SpillValue = MTracker->getSpillMLoc(SpillID);
...
bool InstrRefBasedLDV::pickVPHILoc(
  ...
  for (unsigned Idx = 0; Idx < FirstValue.getLocationOpCount(); ++Idx) {
    ...
    Optional<ValueIDNum> JoinedOpLoc = pickValuePHILoc(
      Idx, MBB, LiveOuts, MOutLocs, BlockOrders);
...
Optional<ValueIDNum>
InstrRefBasedLDV::pickValuePHILoc(
    unsigned DbgOpIdx, const MachineBasicBlock &MBB,
    const LiveIdxT &LiveOuts, FuncValueTable &MOutLocs,
    const SmallVectorImpl<const MachineBasicBlock *> &BlockOrders) {
...
bool InstrRefBasedLDV::depthFirstVLocAndEmit(
  ...
  auto EjectBlock = [&](MachineBasicBlock &MBB) -> void {
    ...
    TTracker->loadInlocs(MBB, MInLocs[BBNum], DbgOpStore, Output[BBNum], NumLocs);
