...
InstrEmitter::EmitDbgInstrRef(SDDbgValue *SD,
  ...
  for (unsigned OpIdx = 0; OpIdx < OpCount; ++OpIdx) {
    ...
    if (DbgOperand.getKind() == SDDbgOperand::VREG) {
      ...
      if (const ConstantInt *CI = dyn_cast<ConstantInt>(V)) {
        if (CI->getBitWidth() > 64)
          MOs.push_back(MachineOperand::CreateCImm(CI));
        else
          MOs.push_back(MachineOperand::CreateImm(CI->getSExtValue()));
      } else if (const ConstantFP *CF = dyn_cast<ConstantFP>(V)) {
        MOs.push_back(MachineOperand::CreateFPImm(CF));
      } else if (isa<ConstantPointerNull>(V)) {
        // Note: This assumes that all nullptr constants are zero-valued.
...
MachineInstr *InstrEmitter::EmitDbgNoLocation(SDDbgValue *SD) {
  ...
  DebugLoc DL = SD->getDebugLoc();
  const MCInstrDesc& Desc = TII->get(SD->isVariadic()
    ? TargetOpcode::DBG_VALUE_LIST
    : TargetOpcode::DBG_VALUE);
  auto MIB = BuildMI(*MF, DL, Desc);
  if (!SD->isVariadic()) {
    MIB.addReg(0U);
    MIB.addReg(0U);
  }
  ...
  if (SD->isVariadic()) {
    for (unsigned OpIdx = 0; OpIdx < SD->getLocationOps().size(); ++OpIdx)
      MIB.addReg(0U);
  }
  return &*MIB;
}
