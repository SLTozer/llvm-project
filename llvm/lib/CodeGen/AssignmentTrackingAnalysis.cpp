...
void AssignmentTrackingLowering::emitDbgValue(
  ...
  dbgs() << "Emitting DbgValue for source: " << *Source << "\n";
...
static AssignmentTrackingLowering::OverlapMap buildOverlapMapAndRecordDeclares(
  ...
  dbgs() << "Fragments we got:\n";
  for (auto &Pair : Map) {
    dbgs() << "Var: " << *FnVarLocs->getVariable(Pair.first).getVariable()
           << "\n";
    for (auto &Frag : Pair.second)
      dbgs() << "  "
             << FnVarLocs->getVariable(Frag).getFragmentOrDefault().OffsetInBits
             << ", "
             << FnVarLocs->getVariable(Frag).getFragmentOrDefault().SizeInBits
             << "\n";
  }
  return Map;
}
