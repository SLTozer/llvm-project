...
struct DVIRecoveryRec {
  ...
  DVIRecoveryRec(DPValue *DPV)
      : DbgRef(DPV), Expr(DPV->getExpression()),
        HadLocationArgList(false) {}
...
static void updateDVIWithLocation(PointerUnion<DbgValueInst *, DPValue *> DbgVal, Value *Location,
                                  SmallVectorImpl<uint64_t> &Ops) {
  auto UpdateImpl = [&](auto *Dbg) {
    assert(
        numLLVMArgOps(Ops) == 0 &&
        "Expected expression that does not contain any DW_OP_llvm_arg operands.");
    Dbg->setRawLocation(ValueAsMetadata::get(Location));
    Dbg->setExpression(DIExpression::get(Dbg->getContext(), Ops));
    Dbg->setExpression(DIExpression::get(Dbg->getContext(), Ops));
  };
  if (isa<DbgValueInst*>(DbgVal))
    UpdateImpl(cast<DbgValueInst*>(DbgVal));
  else
    UpdateImpl(cast<DPValue*>(DbgVal));
}

/// Overwrite DVI with locations placed into a DIArglist.
static void updateDVIWithLocations(PointerUnion<DbgValueInst *, DPValue *> DbgVal,
  ...
  auto UpdateImpl = [&](auto *Dbg) {
    ...
    Dbg->setRawLocation(llvm::DIArgList::get(Dbg->getContext(), ValArrayRef));
    Dbg->setExpression(DIExpression::get(Dbg->getContext(), Ops));
  };
  if (isa<DbgValueInst*>(DbgVal))
    UpdateImpl(cast<DbgValueInst*>(DbgVal));
...
static void UpdateDbgValueInst(DVIRecoveryRec &DVIRec,
  ...
  if (NumLLVMArgs == 0) {
    ...
    updateDVIWithLocation(DVIRec.DbgRef, NewLocationOps[0], NewExpr);
    ...
    updateDVIWithLocation(DVIRec.DbgRef, NewLocationOps[0], ShortenedOps);
  } else {
    // Multiple DW_OP_llvm_arg, so DIArgList is strictly necessary.
    updateDVIWithLocations(DVIRec.DbgRef, NewLocationOps, NewExpr);
  }
  ...
  auto SalvageDbgVal = [&](auto *DbgVal) {
  ...
  if (isa<DbgValueInst*>(DVIRec.DbgRef))
    SalvageDbgVal(cast<DbgValueInst*>(DVIRec.DbgRef));
  else
    SalvageDbgVal(cast<DPValue*>(DVIRec.DbgRef));
}
...
static bool SalvageDVI(llvm::Loop *L, ScalarEvolution &SE,
  ...
  if (isa<DbgValueInst *>(DVIRec.DbgRef) ? !cast<DbgValueInst*>(DVIRec.DbgRef)->isKillLocation() : !cast<DPValue*>(DVIRec.DbgRef)->isKillLocation())
    return false;
  ...
  if (isa<DbgValueInst*>(DVIRec.DbgRef))
    LLVM_DEBUG(dbgs() << "scev-salvage: Updated DVI: " << *cast<DbgValueInst*>(DVIRec.DbgRef) << "\n");
  else
    LLVM_DEBUG(dbgs() << "scev-salvage: Updated DVI: " << *cast<DPValue*>(DVIRec.DbgRef) << "\n");
  return true;
}
