; RUN: opt -passes=callsite-splitting -S < %s | FileCheck %s --implicit-check-not=dbg.value -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -passes=callsite-splitting -S < %s --try-experimental-debuginfo-iterators | FileCheck %s --implicit-check-not=dbg.value -check-prefixes=CHECK,NEWDBG-CHECK
