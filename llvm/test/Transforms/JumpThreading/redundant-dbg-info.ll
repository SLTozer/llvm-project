; RUN: opt -passes=jump-threading -S < %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -passes=jump-threading -S < %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK: call void @llvm.dbg.value({{.+}}, metadata ![[B:[0-9]+]], metadata !DIExpression())
; OLDDBG-CHECK: call void @llvm.dbg.value({{.+}}, metadata ![[B:[0-9]+]], metadata !DIExpression())
; OLDDBG-CHECK-NOT: call void @llvm.dbg.value({{.+}}, metadata ![[B]], metadata !DIExpression())
; NEWDBG-CHECK: #dbg_value { {{.+}}, ![[B:[0-9]+]], !DIExpression()
; NEWDBG-CHECK: #dbg_value { {{.+}}, ![[B:[0-9]+]], !DIExpression()
; NEWDBG-CHECK-NOT: #dbg_value { {{.+}}, ![[B]], !DIExpression()
