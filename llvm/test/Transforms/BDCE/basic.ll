...
; RUN: opt -S -passes='debugify,bdce' < %s | FileCheck %s -check-prefixes=DEBUGIFY,OLDDBG-DEBUGIFY
; RUN: opt -S -passes='debugify,bdce' < %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=DEBUGIFY,NEWDBG-DEBUGIFY
...
entry:
  %call = tail call signext i32 @foo(i32 signext 5) #0
  %and = and i32 %call, 33554432
; OLDDBG-DEBUGIFY: call void @llvm.dbg.value(metadata i32 %call, metadata {{.*}}, metadata !DIExpression(DW_OP_constu, 33554432, DW_OP_and, DW_OP_stack_value))
; NEWDBG-DEBUGIFY: #dbg_value { i32 %call, {{.*}}, !DIExpression(DW_OP_constu, 33554432, DW_OP_and, DW_OP_stack_value)
