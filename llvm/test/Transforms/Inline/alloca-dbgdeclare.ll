...
; RUN: opt -passes='cgscc(inline)' -S < %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
; struct A {
;   int arg0;
;   double arg1[2];
; } a, b;
...
; OLDDBG-CHECK-NEXT:   call void @llvm.dbg.declare(metadata ptr %agg.tmp.sroa.3.i,
; NEWDBG-CHECK-NEXT:   #dbg_declare { ptr %agg.tmp.sroa.3.i,
