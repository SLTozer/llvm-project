...
; RUN: opt -enable-debugify -passes='require<profile-summary>,function(codegenprepare)' -S < %s | FileCheck %s -check-prefixes=DEBUG,OLDDBG-DEBUG
; RUN: opt -enable-debugify -passes='require<profile-summary>,function(codegenprepare)' -S < %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=DEBUG,NEWDBG-DEBUG
...
; OLDDBG-DEBUG-NEXT:    call void @llvm.dbg.value(metadata i1 [[CMP]]
; NEWDBG-DEBUG-NEXT:    #dbg_value { i1 [[CMP]]
; DEBUG-NEXT:    br i1 [[CMP]], label [[SELECT_TRUE_SINK:%.*]], label [[SELECT_END:%.*]], !dbg
; DEBUG:       select.true.sink:
; DEBUG-NEXT:    [[DIV:%.*]] = fdiv float [[A]], [[B:%.*]]
; OLDDBG-DEBUG-NEXT:    call void @llvm.dbg.value(metadata float [[DIV]]
; NEWDBG-DEBUG-NEXT:    #dbg_value { float [[DIV]]
; DEBUG-NEXT:    br label [[SELECT_END]], !dbg
; DEBUG:       select.end:
; DEBUG-NEXT:    [[SEL:%.*]] = phi float [ [[DIV]], [[SELECT_TRUE_SINK]] ], [ 2.000000e+00, [[ENTRY:%.*]] ], !dbg
; OLDDBG-DEBUG-NEXT:    call void @llvm.dbg.value(metadata float [[SEL]]
; NEWDBG-DEBUG-NEXT:    #dbg_value { float [[SEL]]
