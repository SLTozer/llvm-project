; RUN: opt -S -passes=loop-rotate -verify-memoryssa < %s | FileCheck %s --implicit-check-not=dbg.value -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -S -passes=loop-rotate -verify-memoryssa < %s --try-experimental-debuginfo-iterators | FileCheck %s --implicit-check-not=dbg.value -check-prefixes=CHECK,NEWDBG-CHECK
...
; CHECK: entry
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %x
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %y
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %z
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %x
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %y
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %z
; CHECK: if.then.lr.ph:
; CHECK: if.then:
; CHECK-NEXT: %z.tr4 = phi
; CHECK-NEXT: %y.tr3 = phi
; CHECK-NEXT: %x.tr2 = phi
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %z.tr4
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %y.tr3
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %x.tr2
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %z.tr4
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %y.tr3
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %x.tr2
; CHECK:      %call = tail call i32 @tak(i32
; CHECK:      %call9 = tail call i32 @tak(i32
; CHECK:      %call14 = tail call i32 @tak(i32
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %call
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %call9
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %call14
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %call
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %call9
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %call14
entry:
...
; OLDDBG-CHECK-NEXT: tail call void @llvm.dbg.value(metadata i32 %x.tr2
; OLDDBG-CHECK-NEXT: tail call void @llvm.dbg.value(metadata i32 %y.tr3
; OLDDBG-CHECK-NEXT: tail call void @llvm.dbg.value(metadata i32 %z.tr4
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %x.tr2
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %y.tr3
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %z.tr4
...
; OLDDBG-CHECK-NEXT: tail call void @llvm.dbg.value(metadata i32 undef
; OLDDBG-CHECK-NEXT: tail call void @llvm.dbg.value(metadata i32 undef
; OLDDBG-CHECK-NEXT: tail call void @llvm.dbg.value(metadata i32 %z.tr.lcssa
; NEWDBG-CHECK-NEXT: #dbg_value { i32 undef
; NEWDBG-CHECK-NEXT: #dbg_value { i32 undef
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %z.tr.lcssa
entry:
  br label %tailrecurse
...
; CHECK: %dec = add
; OLDDBG-CHECK-NEXT: tail call void @llvm.dbg.value
...
; CHECK: L0.preheader:
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 0,
; NEWDBG-CHECK-NEXT: #dbg_value { i32 0,
; CHECK-NEXT: %cmp = icmp slt i32 0, 0,
; CHECK: L1.preheader:
; CHECK-NEXT: %spec.select3 = phi i32
; CHECK-NEXT: %k.02 = phi i32
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %k.02,
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %k.02,
; CHECK: L0.latch:
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %spec.select3,
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %spec.select3,
entry:
