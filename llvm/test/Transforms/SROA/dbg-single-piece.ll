...
; RUN: opt -passes='sroa<preserve-cfg>' %s -S | FileCheck %s --check-prefixes=CHECK,OLDDBG-CHECK,CHECK-PRESERVE-CFG
; RUN: opt -passes='sroa<modify-cfg>' %s -S | FileCheck %s --check-prefixes=CHECK,OLDDBG-CHECK,CHECK-MODIFY-CFG

;; Duplicate copies for RemoveDIs project, eliminating debug intrinsics
; RUN: opt -passes='sroa<preserve-cfg>' %s -S --try-experimental-debuginfo-iterators | FileCheck %s --check-prefixes=CHECK,NEWDBG-CHECK,CHECK-PRESERVE-CFG
; RUN: opt -passes='sroa<modify-cfg>' %s -S --try-experimental-debuginfo-iterators | FileCheck %s --check-prefixes=CHECK,NEWDBG-CHECK,CHECK-MODIFY-CFG
...
; CHECK-NEXT:  entry:
; OLDDBG-CHECK-NEXT:    call void @llvm.dbg.value(metadata ptr poison, metadata [[META3:![0-9]+]], metadata !DIExpression(DW_OP_LLVM_fragment, 64, 64)), !dbg [[DBG8:![0-9]+]]
; NEWDBG-CHECK-NEXT:    #dbg_value { ptr poison, [[META3:![0-9]+]], !DIExpression(DW_OP_LLVM_fragment, 64, 64), [[DBG8:![0-9]+]]
; CHECK-NEXT:    ret void
;
entry:
