...
; RUN: opt -passes='loop-mssa(simple-loop-unswitch<nontrivial>),verify<loops>' -S < %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -passes='loop-mssa(simple-loop-unswitch<nontrivial>),verify<loops>' -S < %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
;
...
; OLDDBG-CHECK-NEXT:    call void @llvm.dbg.value(metadata i32 poison, metadata [[META3:![0-9]+]], metadata !DIExpression()), !dbg [[DBG8:![0-9]+]]
; NEWDBG-CHECK-NEXT:    #dbg_value { i32 poison, [[META3:![0-9]+]], !DIExpression(), [[DBG8:![0-9]+]]
...
; OLDDBG-CHECK-NEXT:    call void @llvm.dbg.value(metadata i32 [[LV]], metadata [[META3]], metadata !DIExpression()), !dbg [[DBG8]]
; NEWDBG-CHECK-NEXT:    #dbg_value { i32 [[LV]], [[META3]], !DIExpression(), [[DBG8]]
