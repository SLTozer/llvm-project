...
; RUN: opt -passes='debugify,memcpyopt,check-debugify' -S < %s 2>&1 -verify-memoryssa | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -passes='debugify,memcpyopt,check-debugify' -S < %s 2>&1 -verify-memoryssa --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK-NEXT:    call void @llvm.dbg.value(metadata ptr [[AGG_TMP]], metadata [[META9:![0-9]+]], metadata !DIExpression()), !dbg [[DBG12]]
; NEWDBG-CHECK-NEXT:    #dbg_value { ptr [[AGG_TMP]], [[META9:![0-9]+]], !DIExpression(), [[DBG12]]
; CHECK-NEXT:    [[TMP1:%.*]] = load ptr, ptr @a, align 8, !dbg [[DBG13:![0-9]+]]
; OLDDBG-CHECK-NEXT:    call void @llvm.dbg.value(metadata ptr [[TMP1]], metadata [[META11:![0-9]+]], metadata !DIExpression()), !dbg [[DBG13]]
; NEWDBG-CHECK-NEXT:    #dbg_value { ptr [[TMP1]], [[META11:![0-9]+]], !DIExpression(), [[DBG13]]
