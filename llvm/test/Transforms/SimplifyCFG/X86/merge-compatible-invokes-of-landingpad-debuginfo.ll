...
; RUN: opt < %s -passes=debugify,simplifycfg -simplifycfg-require-and-preserve-domtree=1 -sink-common-insts -S | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt < %s -passes='debugify,simplifycfg<sink-common-insts>' -S | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt < %s -passes='debugify,simplifycfg<sink-common-insts>' -S --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK-NEXT:    call void @llvm.dbg.value(metadata i1 [[C0]], metadata [[META9:![0-9]+]], metadata !DIExpression()), !dbg [[DBG12]]
; NEWDBG-CHECK-NEXT:    #dbg_value { i1 [[C0]], [[META9:![0-9]+]], !DIExpression(), [[DBG12]]
...
; OLDDBG-CHECK-NEXT:    call void @llvm.dbg.value(metadata i1 [[C1]], metadata [[META11:![0-9]+]], metadata !DIExpression()), !dbg [[DBG17]]
; NEWDBG-CHECK-NEXT:    #dbg_value { i1 [[C1]], [[META11:![0-9]+]], !DIExpression(), [[DBG17]]
...
; OLDDBG-CHECK: attributes #[[ATTR1:[0-9]+]] = { nocallback nofree nosync nounwind speculatable willreturn memory(none) }
;.
