; RUN: opt < %s -passes=reassociate -S | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt < %s -passes=reassociate -S --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata float poison, metadata [[VAR_X:![0-9]+]], metadata !DIExpression())
; NEWDBG-CHECK: #dbg_value { float poison, [[VAR_X:![0-9]+]], !DIExpression()

; CHECK-LABEL: if.then:
; CHECK-NOT: %add1 = fadd fast float %add, %c
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata float poison, metadata [[VAR_Y:![0-9]+]], metadata !DIExpression())
; NEWDBG-CHECK: #dbg_value { float poison, [[VAR_Y:![0-9]+]], !DIExpression()
; CHECK-LABEL: !0 =
