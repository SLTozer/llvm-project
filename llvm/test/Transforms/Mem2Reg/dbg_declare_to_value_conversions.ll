...
define i64 @foo0(i64 %arg) {
  ...
  ; CHECK-LABEL: @foo0
  ; CHECK-SAME:    (i64 [[arg:%.*]])
  ; OLDDBG-CHECK-NEXT:    dbg.value(metadata i64 [[arg]], {{.*}}, metadata !DIExpression())
  ; NEWDBG-CHECK-NEXT:    #dbg_value { i64 [[arg]], {{.*}}, !DIExpression()
...
define i32 @foo1(ptr %arg) {
  ...
  ; CHECK-LABEL: @foo1
  ; CHECK-SAME:    (ptr [[arg:%.*]])
  ; OLDDBG-CHECK-NEXT:    dbg.value(metadata ptr [[arg]], {{.*}}, metadata !DIExpression(DW_OP_deref))
  ; NEWDBG-CHECK-NEXT:    #dbg_value { ptr [[arg]], {{.*}}, !DIExpression(DW_OP_deref)
...
define i32 @foo2(ptr %arg) {
  ...
  ; CHECK-LABEL: @foo2
  ; OLDDBG-CHECK-NEXT:     dbg.value(metadata ptr undef, {{.*}}, metadata !DIExpression(DW_OP_deref, DW_OP_plus_uconst, 2))
  ; NEWDBG-CHECK-NEXT:     #dbg_value { ptr undef, {{.*}}, !DIExpression(DW_OP_deref, DW_OP_plus_uconst, 2)
  %val = load i32, ptr %arg
  ret i32 %val
}
