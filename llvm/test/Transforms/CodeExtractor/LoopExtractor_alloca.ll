; RUN: opt -passes=debugify,loop-simplify,loop-extract -S < %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -passes=debugify,loop-simplify,loop-extract -S < %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK-NEXT:   call void @llvm.dbg.value(metadata ptr %v1
; NEWDBG-CHECK-NEXT:   #dbg_value { ptr %v1
