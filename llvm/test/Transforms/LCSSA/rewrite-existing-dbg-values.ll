; RUN: opt -S -passes=lcssa < %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -S -passes=lcssa < %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i32 %add, metadata [[VAR:![0-9]+]], metadata !DIExpression())
; NEWDBG-CHECK: #dbg_value { i32 %add, [[VAR:![0-9]+]], !DIExpression()


; CHECK-LABEL: outer.exit:
; CHECK-NEXT: [[PN:%[^ ]*]] = phi i32 [ %add.lcssa, %outer.latch ]
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 [[PN]], metadata [[VAR]], metadata !DIExpression())
; NEWDBG-CHECK-NEXT: #dbg_value { i32 [[PN]], [[VAR]], !DIExpression()
...
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %add, metadata [[VAR2:![0-9]+]], metadata !DIExpression())
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %add, [[VAR2:![0-9]+]], !DIExpression()
...
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 [[PN1]], metadata [[VAR2]], metadata !DIExpression())
; NEWDBG-CHECK-NEXT: #dbg_value { i32 [[PN1]], [[VAR2]], !DIExpression()
; CHECK-NEXT: call void @bar(i32 [[PN1]])

; CHECK-LABEL: for.exit2:
; CHECK-NEXT: [[PN2:%[^ ]*]] = phi i32 [ %add, %for.latch ]
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 [[PN2]], metadata [[VAR2]], metadata !DIExpression())
; NEWDBG-CHECK-NEXT: #dbg_value { i32 [[PN2]], [[VAR2]], !DIExpression()
; CHECK-NEXT: call void @bar(i32 [[PN2]])

; CHECK-LABEL: exit:
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %add, metadata [[VAR2]], metadata !DIExpression())
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %add, [[VAR2]], !DIExpression()
