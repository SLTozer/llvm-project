...
; RUN: opt -S -passes='sroa' %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt --try-experimental-debuginfo-iterators -S -passes='sroa' %s | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i8 %g.coerce0, metadata ![[VAR_STRUCT:[0-9]+]], metadata !DIExpression(DW_OP_LLVM_fragment, 0, 8))
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i64 %g.coerce1, metadata ![[VAR_STRUCT]], metadata !DIExpression(DW_OP_LLVM_fragment, 32, 64))
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i8 %frombool, metadata ![[VAR_BOOL:[0-9]+]], metadata !DIExpression())
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i8 %frombool1, metadata ![[VAR_FRAG:[0-9]+]], metadata !DIExpression(DW_OP_LLVM_fragment, 0, 4))
; NEWDBG-CHECK: #dbg_value { i8 %g.coerce0, ![[VAR_STRUCT:[0-9]+]], !DIExpression(DW_OP_LLVM_fragment, 0, 8)
; NEWDBG-CHECK: #dbg_value { i64 %g.coerce1, ![[VAR_STRUCT]], !DIExpression(DW_OP_LLVM_fragment, 32, 64)
; NEWDBG-CHECK: #dbg_value { i8 %frombool, ![[VAR_BOOL:[0-9]+]], !DIExpression()
; NEWDBG-CHECK: #dbg_value { i8 %frombool1, ![[VAR_FRAG:[0-9]+]], !DIExpression(DW_OP_LLVM_fragment, 0, 4)
