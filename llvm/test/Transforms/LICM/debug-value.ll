; RUN: opt -passes=licm < %s -S | FileCheck %s -check-prefixes=OLDDBG-CHECK
; RUN: opt -aa-pipeline=basic-aa -passes='require<aa>,require<targetir>,require<scalar-evolution>,require<opt-remark-emit>,loop-mssa(licm)' < %s -S | FileCheck %s -check-prefixes=OLDDBG-CHECK

; RUN: opt -passes=licm < %s -S --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=NEWDBG-CHECK
...
; OLDDBG-CHECK: tail call void @llvm.dbg.value
; NEWDBG-CHECK: #dbg_value
