...
; RUN: opt < %s -passes='loop-mssa(licm)' -S | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt < %s -passes='loop-mssa(licm)' -S --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
; RUN: opt -aa-pipeline=tbaa,basic-aa -passes='require<aa>,require<targetir>,require<scalar-evolution>,require<opt-remark-emit>,loop-mssa(licm)' -S %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -aa-pipeline=tbaa,basic-aa -passes='require<aa>,require<targetir>,require<scalar-evolution>,require<opt-remark-emit>,loop-mssa(licm)' -S %s --try-experimental-debuginfo-iterators| FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
;
...
; OLDDBG-CHECK-NEXT:    tail call void @llvm.dbg.value(metadata i32 0, metadata [[META12:![0-9]+]], metadata !DIExpression()), !dbg [[DBG5]]
; NEWDBG-CHECK-NEXT:    #dbg_value { i32 0, [[META12:![0-9]+]], !DIExpression(), [[DBG5]]
