...
; RUN: opt < %s -passes='module(coro-early),cgscc(coro-split,coro-split)' -S | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt --try-experimental-debuginfo-iterators < %s -passes='module(coro-early),cgscc(coro-split,coro-split)' -S | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
;
; This file is based on coro-debug-frame-variable.ll.
; CHECK-LABEL: define void @f(
; CHECK:       %[[frame:.*]] = call {{.*}} @llvm.coro.begin
; OLDDBG-CHECK:       call void @llvm.dbg.value(metadata ptr %[[frame]]
; OLDDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetX:[0-9]*]]))
; NEWDBG-CHECK:       #dbg_value { ptr %[[frame]]
; NEWDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetX:[0-9]*]])
;                                                                   ^ No deref at the end, as this variable ("x") is an array;
;                                                                     its value is its address. The entire array is in the frame.
; OLDDBG-CHECK:       call void @llvm.dbg.value(metadata ptr %[[frame]]
; OLDDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetSpill:[0-9]*]], DW_OP_deref))
; OLDDBG-CHECK:       call void @llvm.dbg.value(metadata ptr %[[frame]]
; OLDDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetI:[0-9]*]], DW_OP_deref))
; OLDDBG-CHECK:       call void @llvm.dbg.value(metadata ptr %[[frame]]
; OLDDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetJ:[0-9]*]], DW_OP_deref))
; NEWDBG-CHECK:       #dbg_value { ptr %[[frame]]
; NEWDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetSpill:[0-9]*]], DW_OP_deref)
; NEWDBG-CHECK:       #dbg_value { ptr %[[frame]]
; NEWDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetI:[0-9]*]], DW_OP_deref)
; NEWDBG-CHECK:       #dbg_value { ptr %[[frame]]
; NEWDBG-CHECK-SAME:    !DIExpression(DW_OP_plus_uconst, [[OffsetJ:[0-9]*]], DW_OP_deref)
...
; CHECK:       init.ready:
; OLDDBG-CHECK:         call void @llvm.dbg.value(metadata ptr %[[frame_alloca]], metadata ![[XVAR_RESUME:[0-9]+]],
; OLDDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetX]])
; NEWDBG-CHECK:         #dbg_value { ptr %[[frame_alloca]], ![[XVAR_RESUME:[0-9]+]],
; NEWDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetX]])
; CHECK:       await.ready:
; OLDDBG-CHECK:         call void @llvm.dbg.value(metadata ptr %[[frame_alloca]], metadata ![[SPILL_RESUME:[0-9]+]]
; OLDDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetSpill]], DW_OP_deref)
; OLDDBG-CHECK:         call void @llvm.dbg.value(metadata ptr %[[frame_alloca]], metadata ![[IVAR_RESUME:[0-9]+]],
; OLDDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetI]], DW_OP_deref)
; OLDDBG-CHECK:         call void @llvm.dbg.value(metadata ptr %[[frame_alloca]], metadata ![[JVAR_RESUME:[0-9]+]],
; OLDDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetJ]], DW_OP_deref)
; NEWDBG-CHECK:         #dbg_value { ptr %[[frame_alloca]], ![[SPILL_RESUME:[0-9]+]]
; NEWDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetSpill]], DW_OP_deref)
; NEWDBG-CHECK:         #dbg_value { ptr %[[frame_alloca]], ![[IVAR_RESUME:[0-9]+]],
; NEWDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetI]], DW_OP_deref)
; NEWDBG-CHECK:         #dbg_value { ptr %[[frame_alloca]], ![[JVAR_RESUME:[0-9]+]],
; NEWDBG-CHECK-SAME:        !DIExpression(DW_OP_deref, DW_OP_plus_uconst, [[OffsetJ]], DW_OP_deref)
;
