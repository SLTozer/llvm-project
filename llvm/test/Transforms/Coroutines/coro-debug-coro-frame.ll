...
; OLDDBG-CHECK:        call void @llvm.dbg.declare(metadata ptr %[[begin]], metadata ![[CORO_FRAME:[0-9]+]], metadata !DIExpression())
; NEWDBG-CHECK:        #dbg_declare { ptr %[[begin]], ![[CORO_FRAME:[0-9]+]], !DIExpression()
;
; CHECK:       define internal fastcc void @f.resume(
; CHECK:       entry.resume:
; CHECK:            %[[FramePtr_RESUME:.*]] = alloca ptr
; OLDDBG-CHECK:            call void @llvm.dbg.declare(metadata ptr %[[FramePtr_RESUME]], metadata ![[CORO_FRAME_IN_RESUME:[0-9]+]], metadata !DIExpression(DW_OP_deref)
; NEWDBG-CHECK:            #dbg_declare { ptr %[[FramePtr_RESUME]], ![[CORO_FRAME_IN_RESUME:[0-9]+]], !DIExpression(DW_OP_deref)
;
