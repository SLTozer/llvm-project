...
; RUN: opt < %s -passes=instcombine -S | FileCheck --check-prefixes=CHECK,OLDDBG-CHECK,DEFAULT %s
; RUN: opt < %s -passes=instcombine --enable-knowledge-retention -S | FileCheck --check-prefixes=CHECK,OLDDBG-CHECK,BUNDLES %s

; RUN: opt < %s -passes=instcombine -S --try-experimental-debuginfo-iterators | FileCheck --check-prefixes=CHECK,NEWDBG-CHECK,DEFAULT %s
; RUN: opt < %s -passes=instcombine --enable-knowledge-retention -S --try-experimental-debuginfo-iterators | FileCheck --check-prefixes=CHECK,NEWDBG-CHECK,BUNDLES %s
...
; OLDDBG-CHECK-NEXT:    tail call void @llvm.dbg.value(metadata i32 5, metadata [[META7:![0-9]+]], metadata !DIExpression()), !dbg [[DBG9:![0-9]+]]
; NEWDBG-CHECK-NEXT:    #dbg_value { i32 5, [[META7:![0-9]+]], !DIExpression(), [[DBG9:![0-9]+]]
; CHECK-NEXT:    store i1 true, ptr poison, align 1
; CHECK-NEXT:    ret void
;
  %cmp1 = icmp eq i8 %x, 0
