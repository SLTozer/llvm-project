...
; RUN: opt -passes=instcombine -S -o - %s | FileCheck %s -check-prefixes=OLDDBG-CHECK
; RUN: opt -passes=instcombine -S -o - %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=NEWDBG-CHECK
...
;OLDDBG-CHECK: call void @llvm.dbg.value(metadata i32 poison
;OLDDBG-CHECK: call void @llvm.dbg.value(metadata i32 -8
;OLDDBG-CHECK: call void @llvm.dbg.value(metadata i32 poison
;NEWDBG-CHECK: #dbg_value { i32 poison
;NEWDBG-CHECK: #dbg_value { i32 -8
;NEWDBG-CHECK: #dbg_value { i32 poison
