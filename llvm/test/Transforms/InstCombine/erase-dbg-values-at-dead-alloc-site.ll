; RUN: opt -S -passes=instcombine %s | FileCheck %s -check-prefixes=RUN-ONCE,OLDDBG-RUN-ONCE
; RUN: opt -S -passes=instcombine %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=RUN-ONCE,NEWDBG-RUN-ONCE
...
;
; RUN-ONCE-LABEL: @t1(
; OLDDBG-RUN-ONCE-NEXT: llvm.dbg.value(metadata i32 %0, metadata [[t1_arg0:![0-9]+]], metadata !DIExpression())
; OLDDBG-RUN-ONCE-NEXT: llvm.dbg.value(metadata ptr poison, metadata [[t1_fake_ptr:![0-9]+]], metadata !DIExpression())
; NEWDBG-RUN-ONCE-NEXT: #dbg_value { i32 %0, [[t1_arg0:![0-9]+]], !DIExpression()
; NEWDBG-RUN-ONCE-NEXT: #dbg_value { ptr poison, [[t1_fake_ptr:![0-9]+]], !DIExpression()
