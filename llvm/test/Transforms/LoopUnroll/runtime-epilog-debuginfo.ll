; RUN: opt -passes=loop-unroll -unroll-runtime -unroll-runtime-epilog -S %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt -passes=loop-unroll -unroll-runtime -unroll-runtime-epilog -S %s --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %add1.i.epil,
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %add1.i.epil,
; CHECK-NEXT: %inc.i.epil = add nuw i32 %i.09.i.epil, 1, !dbg !36
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %inc.i.epil,
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %inc.i.epil,
; OLDDBG-CHECK-NEXT: call void @llvm.dbg.value(metadata i32 %add1.i.epil,
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %inc.i.epil,
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %inc.i.epil,
; NEWDBG-CHECK-NEXT: #dbg_value { i32 %add1.i.epil,
