; RUN: opt -passes='sroa' -S -o - %s | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt --try-experimental-debuginfo-iterators -passes='sroa' -S -o - %s | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
entry:
  ...
  ; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i32 0, metadata ![[A:.*]], metadata !DIExpression())
  ; NEWDBG-CHECK: #dbg_value { i32 0, ![[A:.*]], !DIExpression()
