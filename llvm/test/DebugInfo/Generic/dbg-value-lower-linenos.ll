; RUN: opt < %s -S -passes=mem2reg,instcombine | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt --try-experimental-debuginfo-iterators < %s -S -passes=mem2reg,instcombine | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK-NEXT:  dbg.value(metadata i32 %bar.0,{{.*}}), !dbg ![[UNKNOWN:[0-9]+]]
; NEWDBG-CHECK-NEXT:  #dbg_value { i32 %bar.0,{{.*}}), ![[UNKNOWN:[0-9]+]]
; CHECK-NEXT:  %totest = load
; CHECK-NEXT:  %add = add i32 %bar.0
; OLDDBG-CHECK-NEXT:  dbg.value(metadata i32 %add, {{.*}}), !dbg ![[UNKNOWN]]
; NEWDBG-CHECK-NEXT:  #dbg_value { i32 %add, {{.*}}), ![[UNKNOWN]]
; CHECK-NEXT:  %cond = icmp ult
; CHECK-NEXT:  br i1 %cond, label %bb1, label %bb2
;
; CHECK-LABEL: bb2:
; CHECK-NEXT:  %toret = add i32 %bar.0, 3
; OLDDBG-CHECK-NEXT:  dbg.value(metadata i32 %toret, {{.*}}), !dbg ![[UNKNOWN]]
; NEWDBG-CHECK-NEXT:  #dbg_value { i32 %toret, {{.*}}), ![[UNKNOWN]]
