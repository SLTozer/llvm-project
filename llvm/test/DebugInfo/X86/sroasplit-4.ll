; RUN: opt -passes='sroa' < %s -S -o - | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt --try-experimental-debuginfo-iterators -passes='sroa' < %s -S -o - | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
;
; Test that recursively splitting an alloca updates the debug info correctly.
; CHECK: %[[T:.*]] = load i64, ptr @t, align 8
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i64 %[[T]], metadata ![[Y:.*]], metadata !DIExpression(DW_OP_LLVM_fragment, 0, 64))
; NEWDBG-CHECK: #dbg_value { i64 %[[T]], ![[Y:.*]], !DIExpression(DW_OP_LLVM_fragment, 0, 64)
; CHECK: %[[T1:.*]] = load i64, ptr @t, align 8
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i64 %[[T1]], metadata ![[Y]], metadata !DIExpression(DW_OP_LLVM_fragment, 64, 64))
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i64 %[[T]], metadata ![[R:.*]], metadata !DIExpression(DW_OP_LLVM_fragment, 192, 64))
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata i64 %[[T1]], metadata ![[R]], metadata !DIExpression(DW_OP_LLVM_fragment, 256, 64))
; NEWDBG-CHECK: #dbg_value { i64 %[[T1]], ![[Y]], !DIExpression(DW_OP_LLVM_fragment, 64, 64)
; NEWDBG-CHECK: #dbg_value { i64 %[[T]], ![[R:.*]], !DIExpression(DW_OP_LLVM_fragment, 192, 64)
; NEWDBG-CHECK: #dbg_value { i64 %[[T1]], ![[R]], !DIExpression(DW_OP_LLVM_fragment, 256, 64)
;
; struct p {
;   __SIZE_TYPE__ s;
;   __SIZE_TYPE__ t;
