; RUN: opt -S -debugify -codegenprepare %s -o - | FileCheck %s --check-prefixes=DEBUGIFY,OLDDBG-DEBUGIFY
; RUN: opt -S -debugify -codegenprepare %s -o - --try-experimental-debuginfo-iterators | FileCheck %s --check-prefixes=DEBUGIFY,NEWDBG-DEBUGIFY
;
...
; OLDDBG-DEBUGIFY-NEXT: call void @llvm.dbg.value(metadata i32 %add,
; NEWDBG-DEBUGIFY-NEXT: #dbg_value { i32 %add,
; DEBUGIFY-NEXT: %sextadd = sext
; OLDDBG-DEBUGIFY-NEXT: call void @llvm.dbg.value(metadata i64 %sextadd,
; NEWDBG-DEBUGIFY-NEXT: #dbg_value { i64 %sextadd,
; DEBUGIFY-NEXT: %arrayidx = getelementptr
; OLDDBG-DEBUGIFY-NEXT: call void @llvm.dbg.value(metadata ptr %arrayidx,
; NEWDBG-DEBUGIFY-NEXT: #dbg_value { ptr %arrayidx,
; DEBUGIFY-NEXT: %res = load i8,
; OLDDBG-DEBUGIFY-NEXT: call void @llvm.dbg.value(metadata i8 %res,
; NEWDBG-DEBUGIFY-NEXT: #dbg_value { i8 %res,
