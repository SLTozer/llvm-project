; RUN: opt %s -passes='cgscc(function(sroa,instcombine),inline),function(instcombine,sroa),verify' -S -o - | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt --try-experimental-debuginfo-iterators %s -passes='cgscc(function(sroa,instcombine),inline),function(instcombine,sroa),verify' -S -o - | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
;
...
; OLDDBG-CHECK: llvm.dbg.value(metadata i32 %[[RESULT]], metadata [[METADATA_IDX1:![0-9]+]]
; NEWDBG-CHECK: #dbg_value { i32 %[[RESULT]], [[METADATA_IDX1:![0-9]+]]
; CHECK: ret
; CHECK: DICompileUnit
