; RUN: opt  -passes=instcombine %s -S | FileCheck %s -check-prefixes=OLDDBG-CHECK
; RUN: opt --try-experimental-debuginfo-iterators  -passes=instcombine %s -S | FileCheck %s -check-prefixes=NEWDBG-CHECK
;
; Generate me from:
...
; OLDDBG-CHECK: call void @llvm.dbg.declare(metadata ptr %vla, metadata ![[VLA:.*]], metadata {{.*}})
; NEWDBG-CHECK: #dbg_declare { ptr %vla, ![[VLA:.*]], {{.*}})
; The VLA alloca and following store into the array should not be lowered to like this:
; OLDDBG-CHECK-NOT:  call void @llvm.dbg.value(metadata float %r, metadata ![[VLA]])
; NEWDBG-CHECK-NOT:  #dbg_value { float %r, ![[VLA]])
