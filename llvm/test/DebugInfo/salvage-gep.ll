; RUN: opt %s -passes=dce -S | FileCheck %s -check-prefixes=CHECK,OLDDBG-CHECK
; RUN: opt %s -passes=dce -S --try-experimental-debuginfo-iterators | FileCheck %s -check-prefixes=CHECK,NEWDBG-CHECK
...
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata !DIArgList(ptr %ptr, i64 %offset),
; NEWDBG-CHECK: #dbg_value { !DIArgList(ptr %ptr, i64 %offset),
; CHECK-SAME: ![[VAR_OFFSET_PTR:[0-9]+]],
; OLDDBG-CHECK-SAME: !DIExpression(DW_OP_LLVM_arg, 0, DW_OP_LLVM_arg, 1, DW_OP_constu, 8, DW_OP_mul, DW_OP_plus, DW_OP_plus_uconst, 4, DW_OP_stack_value))
; NEWDBG-CHECK-SAME: !DIExpression(DW_OP_LLVM_arg, 0, DW_OP_LLVM_arg, 1, DW_OP_constu, 8, DW_OP_mul, DW_OP_plus, DW_OP_plus_uconst, 4, DW_OP_stack_value)

;; The variable offset should be ignored, as it applies to a type of width 0,
;; leaving only the constant offset.
; OLDDBG-CHECK: call void @llvm.dbg.value(metadata ptr %zptr,
; NEWDBG-CHECK: #dbg_value { ptr %zptr,
; CHECK-SAME: ![[VAR_ZERO_PTR:[0-9]+]],
; OLDDBG-CHECK-SAME: !DIExpression(DW_OP_plus_uconst, 44, DW_OP_stack_value))
; NEWDBG-CHECK-SAME: !DIExpression(DW_OP_plus_uconst, 44, DW_OP_stack_value)
