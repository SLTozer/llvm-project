...
entry:
  %call = tail call i32 (...) @baz() #2, !!dbgLoc(srcLoc: 3, atom: [1, 2])
  %mul = mul nsw i32 %call, %a, !!dbgLoc(srcLoc: 0)
  %add = add nsw i32 %mul, %b, !!dbgLoc(srcLoc: 1, atom: [3, 2])
  ret i32 %add, !!dbgLoc(srcLoc: 2, atom: [3, 1])
}
...
entry:
  %cmp11 = icmp sgt i32 %i, 0, !!dbgLoc(srcLoc: 4, atom: [13, 1])
  br i1 %cmp11, label %for.body, label %cleanup2, !!dbgLoc(srcLoc: 5, atom: [14, 1])

for.cond:                                         ; preds = %for.body
  %inc = add nuw nsw i32 %l.014, 1, !!dbgLoc(srcLoc: 6, atom: [9, 2])
  %exitcond.not = icmp eq i32 %inc, %i, !!dbgLoc(srcLoc: 4, atom: [2, 1])
  br i1 %exitcond.not, label %cleanup2, label %for.body, !!dbgLoc(srcLoc: 5, atom: [3, 1]), !!loop 0
...
for.body:                                         ; preds = %entry, %for.cond
  ...
  %call.i = tail call i32 (...) @baz() #2, !!dbgLoc(srcLoc: 3, inlinedAt: 0, atom: [1, 2])
  %mul.i = mul nsw i32 %call.i, %x.addr.013, !!dbgLoc(srcLoc: 0, inlinedAt: 0)
  %add.i = add nsw i32 %mul.i, %y.addr.012, !!dbgLoc(srcLoc: 1, inlinedAt: 0, atom: [3, 2])
  %rem = srem i32 %add.i, 5, !!dbgLoc(srcLoc: 1)
  %cmp1.not = icmp eq i32 %rem, 0, !!dbgLoc(srcLoc: 2, atom: [5, 2])
...
cleanup2:                                         ; preds = %for.body, %for.cond, %entry
  ...
  ret i32 %0, !!dbgLoc(srcLoc: 3, atom: [12, 1])
}
...
!37 = distinct !DIFunctionLocalMetadata(
  srcLocs: [
    !!srcLoc(line: 6, column: 12, scope: !12), ; 0
    !!srcLoc(line: 6, column: 16, scope: !12), ; 1
    !!srcLoc(line: 6, column: 3, scope: !12), ; 2
    !!srcLoc(line: 5, column: 11, scope: !12) ; 3
  ],
)
!38 = distinct !DIFunctionLocalMetadata(
  srcLocs: [
    !!srcLoc(line: 11, column: 16, scope: !20), ; 0
    !!srcLoc(line: 12, column: 14, scope: !20), ; 1
    !!srcLoc(line: 12, column: 18, scope: !20), ; 2
    !!srcLoc(line: 18, column: 1, scope: !20), ; 3
    !!srcLoc(line: 10, column: 21, scope: !20), ; 4
    !!srcLoc(line: 10, column: 3, scope: !20), ; 5
    !!srcLoc(line: 10, column: 26, scope: !20), ; 6
    !!srcLoc(line: 16, column: 3, scope: !20) ; 7
  ],
  inlinedCallLocs: [
    !!inlinedCallLoc(dbgLoc: !!dbgLoc(srcLoc: 0), inlinedFLMD: !37) ; 0
  ],
  loops: [
    !!loop(start: !!dbgLoc(srcLoc: 5), end: !!dbgLoc(srcLoc: 7), properties: !29) ; 0
  ]
)
