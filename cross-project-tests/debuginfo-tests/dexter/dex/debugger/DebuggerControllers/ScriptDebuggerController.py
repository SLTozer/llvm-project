...
class ScriptDebuggerController(DebuggerControllerBase):
    ...
    def _run_debugger_custom(self, cmdline):
        ...
        while not self.debugger.is_finished:
            ...
            ## Use the acquired frame information to determine which !where nodes match a stack frame, along with the
            ## matching frame index and any child expect nodes.
            ...
            def get_active_wheres(where: Where, scope: Scope):
                ...
                matching_frame_idx = next(
                    (
                        frame_idx
                        for frame_idx, frame in reversed(
                            list(enumerate(step_info.frames))
                        )
            ...
            def get_active_expects(expect: Expect, expected_value, scope: Scope):
                ...
                if scope.where in active_where_expects:
                    active_where_expects[scope.where][1].append(expect)

            script.visit_script(
                visit_where=get_active_wheres, visit_expect=get_active_expects
            )
            ...
            watches = [
                ...
                if (watch := expect.get_watched_expr()) and frame_idx == 0
            ]
            ...
            if any(
                frame_idx == 0 for frame_idx, values in active_where_expects.values()
            ):
